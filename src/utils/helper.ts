...
export function timeFormatter(
	...
	const stringTime = new Date(
		date.replace(/^(\d{4})(\d\d)(\d\d)(\d\d)(\d\d)$/, '$4:$5:$6 $2/$3/$1')
	);
...
export function getWeatherData(parsedXML: any): Cuaca {
	...
	for (let i = 0; i < rest.area.length; i++) {
		...
		for (let j = 0; j < 12; j++) {
			weatherArray.push({
				waktu: timeFormatter(
					rest.area[i].parameter[6].timerange[j]['@_datetime']
				),
				cuaca: weatherCodeToString(
					rest.area[i].parameter[6].timerange[j].value['#text']
				),
				suhu: {
					celsius: {
